...
void RelationshipAddWriteDelete()
{
  ...
  for (int i = 0; i < cycles; ++i) {
    ...
    for (int j = 0; j < rootBranchCount; ++j) {
      ...
      const World::Member& rootMember = space.GetMember(rootIds[j]);
      const Ds::Vector<World::MemberId>& children = rootMember.Children();
      for (World::MemberId childId : children) {
        space.Get<C>(childId).Write(1.0f);
        const World::Member& childMember = space.GetMember(childId);
        const Ds::Vector<World::MemberId>& grandchildren =
          childMember.Children();
        for (World::MemberId grandchildId : grandchildren) {
...
void Random()
{
  ...
  auto randomEnsure = [&]()
  {
    ...
    // clang-format off
    switch (type) {
    case 0: if(!space.Has<A>(id)) { space.Add<A>(id); } break;
    case 1: if(!space.Has<B>(id)) { space.Add<B>(id); } break;
    case 2: if(!space.Has<C>(id)) { space.Add<C>(id); } break;
    case 3: if(!space.Has<D>(id)) { space.Add<D>(id); } break;
    }
    // clang-format on
  };
